...
function normalizeOrderPricing(pricing = {}, fallbackCurrency = "SAR") {
...
function buildSubscriptionPricingSummary(subscription) {
  const snapshotPricing = subscription && subscription.contractSnapshot && subscription.contractSnapshot.pricing
  ...
  return buildMoneySummary({
    ...normalized,
    currency: normalizeCurrencyValue(subscription && subscription.checkoutCurrency ? subscription.checkoutCurrency : "SAR"),
  });
}
...
async function loadSubscriptionSummaryCatalog(subscriptions, lang) {
  const { addonIds, planIds } = collectSubscriptionCatalogIds(subscriptions);
  const [addonDocs, planDocs] = await Promise.all([
    addonIds.length
  ...
  return res.status(200).json({
    ...
    data: {
      ...
      orders: filteredOrders.map((order) => ({
        pricingSummary: buildMoneySummary({
          basePriceHalala: normalizeOrderPricing(order.pricing || {}).basePrice,
          vatPercentage: normalizeOrderPricing(order.pricing || {}).vatPercentage,
          vatHalala: normalizeOrderPricing(order.pricing || {}).vatAmount,
          totalPriceHalala: normalizeOrderPricing(order.pricing || {}).total,
          currency: normalizeOrderPricing(order.pricing || {}).currency,
        }),
        id: String(order._id),
...
async function getTodayReport(req, res) {
  ...
  const payload = {
    ...
    recentSubscriptions: recentTodaySubscriptions.map((subscription) => {
      ...
      const pricingSummary = buildSubscriptionPricingSummary(subscription);
      return {
        ...
        amountDisplay: formatAmountDisplay(pricingSummary.totalPriceHalala, subscription.checkoutCurrency, lang),
        pricingSummary,
      };
    }),
  };
...
async function persistNormalizedSettings(normalizedSettings) {
  ...
  if (Object.prototype.hasOwnProperty.call(normalizedSettings, "restaurant_open_time")) {
    operations.push(persistSettingValue("restaurant_open_time", normalizedSettings.restaurant_open_time));
    persisted.restaurant_open_time = normalizedSettings.restaurant_open_time;
  }

  if (Object.prototype.hasOwnProperty.call(normalizedSettings, "restaurant_close_time")) {
...
function normalizeSettingsPatchPayloadOrThrow(payload) {
  ...
  if (Object.prototype.hasOwnProperty.call(payload, "restaurant_open_time")) {
    normalized.restaurant_open_time = normalizeCutoffTimeOrThrow(payload.restaurant_open_time);
  }
  ...
  return res.status(200).json({
    ...
    data: {
      ...
      recentSubscriptions: recentSubscriptions.map((subscription) => {
        ...
        const pricingSummary = buildSubscriptionPricingSummary(subscription);
        const amountMinorUnits = Number(pricingSummary.totalPriceHalala || 0);
      ...
      recentOrders: recentOrders.map((order) => {
        ...
        const pricing = normalizeOrderPricing(order.pricing && typeof order.pricing === "object" ? order.pricing : {});
