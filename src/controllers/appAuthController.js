...
async function getTodayPickup(req, res) {
  try {
    ...
    const days = await SubscriptionDay.find({
      ...
      date: today,
      status: { $in: ["open", "locked", "in_preparation", "ready_for_pickup", "fulfilled", "no_show", "consumed_without_preparation", "canceled_at_branch"] },
    })
      .select("status pickupCode pickupRequested pickupPreparationStartedAt pickupPreparedAt dayEndConsumptionReason lockedSnapshot fulfilledSnapshot")
      .lean();
    if (!days.length) {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        pickupRequested: Boolean(selectedDay.pickupRequested),
        pickupPrepared: Boolean(selectedDay.pickupPreparationStartedAt || selectedDay.pickupPreparedAt || ["ready_for_pickup", "fulfilled", "no_show"].includes(selectedDay.status)),
        dayEndConsumptionReason: selectedDay.dayEndConsumptionReason || null,
      },
    });
  } catch (err) {
    return handleError(res, err);
