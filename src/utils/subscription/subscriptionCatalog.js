...
function resolveQuoteSummary(quote, lang) {
  ...
  const addonItems = (quote.addonItems || []).map((item) => {
    ...
    return {
      id: String(addon._id || ""),
      addonPlanId: String(item.addonPlanId || addon._id || ""),
      name: pickLang(addon.name, lang),
      qty,
      quantityPerDay: Number(item.quantityPerDay || qty || 1),
      daysCount: Number(item.daysCount || durationDays || planDaysCount || 0),
      ...
      billingUnit: billingUnitMap[billingMode] || "item",
      durationDays,
      unitPlanPriceHalala: Number(item.unitPlanPriceHalala || unit.halala),
      unitPriceHalala: unit.halala,
      unitPriceSar: unit.sar,
