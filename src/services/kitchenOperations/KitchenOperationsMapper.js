...
function mapOrderToRow(order, context = {}) {
  ...
  const kitchenDetails = buildOrderKitchenDetailsPayload(order, "ar", context.catalogMaps || {});
  const row = { id: stringifyId(order._id), entityType: "order", reference: formatReference("ORD", date, order._id), customer: buildCustomerFromOrder(order), date, mode, modeLabel: MODE_LABELS[mode] || mode, fulfillmentType: mode === "pickup" ? "branch_pickup" : "delivery", kitchenDetails, ...buildKitchenProjection(kitchenDetails), delivery: buildDeliveryPayload(order.deliveryRecord || null, { date, address: order.deliveryAddress || (order.delivery && order.delivery.address) || null, window: order.deliveryWindow || null, zoneId: order.delivery && order.delivery.zoneId ? order.delivery.zoneId : null }), pickup: mode === "pickup" ? buildPickupPayload({ day: order }) : null, paymentValidity: { paymentRequired: true, paymentStatus: order.paymentStatus || "initiated", paymentApplied: String(order.paymentStatus || "") === "paid", pendingUnpaid: String(order.paymentStatus || "") !== "paid", superseded: false, revisionMismatch: false, canPrepare: String(order.paymentStatus || "") === "paid" && order.status === "confirmed", canFulfill: String(order.paymentStatus || "") === "paid" && ["out_for_delivery", "ready_for_pickup"].includes(order.status), reason: String(order.paymentStatus || "") === "paid" ? null : "ORDER_PAYMENT_REQUIRED" }, timeWindow: parseTimeWindow(order.deliveryWindow), items, status: statusMeta.status, statusLabel: statusMeta.statusLabel, progress: statusMeta.progress, actions: [], badges: { locked: ["in_preparation", "out_for_delivery", "ready_for_pickup", "fulfilled"].includes(order.status), assignedByKitchen: false, pickupRequested: false }, verification: buildVerification(order, mode), ui: { layout: "table" }, timing: buildTiming(order.createdAt || order.confirmedAt || order.fulfilledAt || null), meta: { subscriptionId: null, orderId: stringifyId(order._id), dayId: null }, operationFlags: { creditsDeducted: false, pickupCodeIssued: false, pickupVerified: false }, rawStatus: order.status, sortStatusOrder: 0, branchId: null };
  row.actions = resolveActions(row);
  return row;
}
