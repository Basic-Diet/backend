...
async function performDaySelectionUpdate({ userId, subscriptionId, date, selections = [], premiumSelections = [], mealSlots, contractVersion, requestedOneTimeAddonIds, runtime, appendOnly = false }) {
  ...
  try {
       ...
       for (const sel of day.addonSelections) {
         if (sel.source === "subscription" || sel.source === "pending_payment" || sel.source === "paid") {
           subInSession.addonSelections.push({
             ...
             catalogAvailable: sel.catalogAvailable === true,
             catalogActive: sel.catalogActive === true,
             liveCatalogAvailable: sel.liveCatalogAvailable === true,
             liveCatalogActive: sel.liveCatalogActive === true,
             selectable: sel.selectable === true,
             selectionAvailable: sel.selectionAvailable === true,
             disabled: sel.disabled === true,
             disableReason: sel.disableReason || null,
    ...
    return {
      ...
      addonSubscriptionAllowances: finalCommercialState.addonSubscriptionAllowances,
...
async function performDaySelectionValidation({
  ...
  return {
    ...
    addonSubscriptionAllowances: derivedDraftState.addonSubscriptionAllowances,
    premiumExtraPayment: derivedDraftState.premiumExtraPayment,
