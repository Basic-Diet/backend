...
const productRows = [
  ...
  { key: "fruit_salad", category: "custom_order", itemType: "fruit_salad", name: name("سلطة فواكه", "Fruit Salad"), pricingModel: "fixed", priceHalala: 1700, defaultWeightGrams: 150, groups: [["fruits", 9, 9]] },
  { key: "greek_yogurt", category: "custom_order", itemType: "greek_yogurt", name: name("زبادي يوناني", "Greek Yogurt"), pricingModel: "fixed", priceHalala: 1700, defaultWeightGrams: 200, groups: [["fruits", 5, 5], ["nuts", 0, 3]] },
  { key: "green_salad", category: "custom_order", itemType: "green_salad", name: name("سلطة خضرا", "Green Salad"), pricingModel: "per_100g", priceHalala: 1500, groups: [["leafy_greens", 2, 2], ["vegetables_legumes", 0, 19], ["sauces", 1, 1]] },
  ...[
...
async function seedOneTimeMenu({ actor = { role: "script" }, notes = "Seed one-time pickup menu" } = {}) {
  ...
  for (const [groupKey, groupDefinition] of Object.entries(groupDefinitions)) {
    const group = await MenuOptionGroup.findOneAndUpdate(
      { key: groupKey },
      { $set: { key: groupKey, name: groupDefinition.name, isActive: true, sortOrder: groupSort, publishedAt: now } },
      { upsert: true, new: true }
    );
    ...
    for (let optionIndex = 0; optionIndex < groupDefinition.options.length; optionIndex += 1) {
      ...
      const option = await MenuOption.findOneAndUpdate(
        ...
        { $set: { groupId: group._id, key: optionSeedKey, name: optionName, extraWeightUnitGrams: groupKey === "proteins" ? 50 : 0, isActive: true, sortOrder: optionSort, publishedAt: now } },
        { upsert: true, new: true }
      );
  ...
  return { products: productRows.length, categories: categoryRows.length };
}
...
async function main() {
  ...
  console.log(`Seeded one-time menu: ${result.products} products`);
