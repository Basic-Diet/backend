...
const LEGACY_PUBLIC_PRODUCT_UI_FIELDS = [
  "cardVariant",
  "badge",
  "ctaLabel",
  "imageRatio",
...
function assertCleanPublicProductUi(product, expectedCardSize) {
  assert.deepStrictEqual(product.ui, { cardSize: expectedCardSize }, `${product.key} exposes only public cardSize`);
  for (const field of LEGACY_PUBLIC_PRODUCT_UI_FIELDS) {
...
(async function run() {
  ...
  try {
    ...
    await test("seed-one-time-menu publishes the final Basic Diet dynamic menu", async () => {
      ...
      assertCleanPublicCategoryUi(categoriesByKey.get("custom_order"));
      assertCleanPublicCategoryUi(categoriesByKey.get("light_options"));
      ...
      assertCleanPublicProductUi(basicSalad, "large");
      ...
      assertCleanPublicProductUi(fruitSalad, "medium");
      ...
      assertCleanPublicProductUi(greekYogurt, "medium");
      ...
      assertCleanPublicProductUi(greenSalad, "medium");
    ...
    await test("Dashboard menu generates immutable catalog keys and exposes ui metadata", async () => {
      ...
      assertCleanPublicProductUi(publicProduct, "medium");
      ...
      assertCleanPublicCategoryUi(fallbackCategory);
      ...
      assertCleanPublicProductUi(fallbackProduct, "medium");
    ...
    await test("GET /api/orders/menu keeps default response stable and exposes opt-in publicMenuV2", async () => {
      ...
      assert.deepStrictEqual(section.ui, {});
