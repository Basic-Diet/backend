...
async function runTests() {
  ...
  const juiceAddon = new Addon({ name: { en: "Juice Addon" }, category: "juice", priceHalala: 0, pricingMode: "fixed", menuProductIds: [juiceProduct._id] });
  await juiceAddon.save({ validateBeforeSave: false });

  const date = "2026-08-01";

  // Test 1: Full Coverage without maxPerDay
  try {
    console.log("Test 1: Full Coverage (7 requested, 25 remaining)");
    ...
    console.log("✅ Test 1 Passed: 7 items successfully covered by balance, ignoring maxPerDay=2.");
  } catch (err) {
  ...
  try {
    console.log("\nTest 2: Partial Coverage (7 requested, 3 remaining)");
    ...
    if (result2.addonSummary.inclusiveCount !== 3 || result2.addonSummary.pendingPaymentCount !== 4) {
      throw new Error(`Expected 3 inclusive, 4 pending. Got ${result2.addonSummary.inclusiveCount} inclusive, ${result2.addonSummary.pendingPaymentCount} pending.`);
    }
    console.log("✅ Test 2 Passed: 3 items covered by balance, 4 items routed to pending_payment.");
  } catch (err) {
    console.error("❌ Test 2 Failed:", err.message);
  }

  // Test 3: Missing Balance Blocking
  try {
    console.log("\nTest 3: Missing Balance Blocking (No addonBalance bucket but entitlement exists)");
    ...
    try {
      ...
      if (e.code === "ADDON_BALANCE_MISSING_REVIEW_REQUIRED") {
        threwError = true;
      } else {
        throw e;
      }
    ...
    if (!threwError) {
      throw new Error(`Expected ADDON_BALANCE_MISSING_REVIEW_REQUIRED error, but none was thrown.`);
    }
    console.log("✅ Test 3 Passed: Validation correctly blocked with ADDON_BALANCE_MISSING_REVIEW_REQUIRED.");
  } catch (err) {
