...
async function seedCatalog() {
  ...
  const [proteinsGroup, carbsGroup, leafyGreens, vegetablesLegumes] = await Promise.all([
    ...
    MenuOptionGroup.create({ key: "leafy_greens", name: { en: "Leafy Greens", ar: "Leafy Greens" }, publishedAt: now }),
    MenuOptionGroup.create({ key: "vegetables_legumes", name: { en: "Vegetables", ar: "Vegetables" }, publishedAt: now }),
  ]);
  ...
  // The default visual template builder generates allowedOptionKeys for the salad proteins group.
  // Validation requires every key in allowedOptionKeys to have a ProductGroupOption entry for the salad.
  // Seed all salad-allowed protein options so validation passes (mirrors production setup).
  const saladAllowedProteinKeys = [
    "boiled_eggs", "tuna", "chicken_fajita", "spicy_chicken", "italian_spiced_chicken",
    "chicken_tikka", "asian_chicken", "chicken_strips", "grilled_chicken", "mexican_chicken", "fish_fillet",
  ];
  let saladSortOrder = 1;
  for (const proteinOption of proteins) {
    if (saladAllowedProteinKeys.includes(proteinOption.key)) {
      await ProductGroupOption.create({
        productId: premiumLargeSalad._id,
        groupId: proteinsGroup._id,
        optionId: proteinOption._id,
        sortOrder: saladSortOrder++,
      });
    }
  }
...
async function main() {
  ...
  try {
    ...
    // builderCatalogV2 is always generated by CatalogService when catalog data exists and is included in the response.
    // The important assertion is that the v3 planner contract is correct.
    if (res.body.data.builderCatalogV2) {
