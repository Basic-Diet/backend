...
// TODO: MenuOption has no nutrition field. Keep salad-only protein calories here until canonical option nutrition is supported.
const saladProteinRows = [
...
const customSaladAllowedOptions = {
  ...
  proteins: saladProteinRows.map((row) => row.key),
...
const groupDefinitions = [
  {
    ...
    options: [
      ...
      ...saladProteinRows.filter((row) => !["beef_steak", "shrimp", "salmon"].includes(row.key)),
    ],
  },
  {
    key: "carbs",
    ...
    for (let optionIndex = 0; optionIndex < groupDef.options.length; optionIndex += 1) {
      ...
      const option = await upsertByMode(
        ...
        { groupId: group._id, key: optionDef.key },
        {
          ...
          extraPriceHalala: optionDef.extraPriceHalala ?? 0,
...
async function seedBuilderCompatibilityMirrors({ optionMap, builderCategoryMap, sync = false }) {
  for (const groupDef of groupDefinitions) {
    for (let optionIndex = 0; optionIndex < groupDef.options.length; optionIndex += 1) {
      ...
      if (groupDef.key === "proteins" && !optionDef.ruleTags?.includes("salad_only")) {
        ...
        if (optionDef.premiumKey) payload.premiumKey = optionDef.premiumKey;
